...
public:
	...
	STDMETHOD(Open)(D3D10_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID *ppData, UINT *pBytes)
	{
		...
		if (g_sharedHeaderPath.empty())
			g_sharedHeaderPath = filePath;
...
int main(int argc, char ** argv)
{
	...
	bool isHeader = endsWith(argv[1], ".fxh");

	//Validate input file type
	if (!endsWith(argv[1], ".fx") && !isHeader)
	{
	...
	//Save blob file
	{
		...
		if (!isHeader)
		{
			...
			size_t hash = g_sharedHeaderPath.empty() ? 0 : hasher(g_sharedHeaderPath);

			filestr.write((char*)&hash, sizeof(size_t));
		}
